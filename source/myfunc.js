...
export const smsg = (sock, m, store) => {
  ...
  m.reply = (text, chatId = m.chat, options = {}) => {
    if (Buffer.isBuffer(text)) {
      ...
      return sock.sendText(chatId, text, m, { mentions: mentionIds, ...options });
    }
  };

  m.copy = () => smsg(sock, JSON.parse(JSON.stringify(m)));
  ...
  sock.appenTextMessage = async (text, chatUpdate) => {
    let messages = await generateWAMessage(
      ...
      { userJid: sock.user.id, quoted: m.quoted && m.quoted.fakeObj }
    );
