...
export default async (sock, m) => {
  try {
    ...
    if (isGroup) {
      groupMetadata = await sock.groupMetadata(chat).catch(() => ({}));
    ...
    if (reactions) {
      if (["😂"].includes(reactions)) {
        sock.sendMessage(m.chat, { text: "*KWKWKWKWK😹*" }, { quoted: null });
      }
    }
    ...
    const reaction = async (jid, emoji) => {
      sock.sendMessage(jid, { react: { text: emoji, key: m.key } });
    };
    ...
    const plugins = await x.loader(path.resolve(__dirname, "../plugins"));
    ...
    switch (commands) {
      case "mode": {
        ...
        reply(`🤖 Bot Mode: ${sock.public ? "Public" : "Self"}`);
        break;
      }
      ...
      case "s": {
        ...
        try {
          if (/image/.test(mime)) {
            ...
            await makeStickerFromUrl(imageUrl, sock, m, reply);
            ...
            await makeStickerFromUrl(videoUrl, sock, m, reply);
          }
        } catch (error) {
          console.error(error);
      ...
      case "bot": {
        await sock.sendMessage(m.chat, { requestPhoneNumber: {} });
        break;
      }
      ...
      case "readviewonce": {
        if (!m.quoted)
          return sock.sendMessage(m.chat, { text: "reply pesan viewOnce nya!" }, { quoted: m });

        const msg = m.quoted.message || m.quoted.fakeObj.message;
        const type = Object.keys(msg)[0];

        if (!msg[type].viewOnce && m.quoted.mtype !== "viewOnceMessageV2") {
          return sock.sendMessage(m.chat, { text: "Pesan itu bukan viewonce!" }, { quoted: m });
        }
