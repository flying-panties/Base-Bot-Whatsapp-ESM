...
async function startServer() {
  const child = async () => {
    ...
    if (!sock.authState.creds.registered) {
      ...
      const codeResult = await sock.requestPairingCode(phoneNumber, customCode);
	...
	sock.ev.on("messages.upsert", async (chatUpdate) => {
      try {
        ...
        m = smsg(sock, m);
        if (handleMessage) {
          await handleMessage(sock, m, chatUpdate);
        } else {
...
sock.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
  try {
    ...
    sock.sendText = (jid, teks, quoted = "", options = {}) => sock.sendMessage(jid, { text: teks, ...options }, { quoted, ...options });
    ...
    sock.sendAudio = async (jid, buff, options = {}) => {
    ...
    sock.sendVideo = async (jid, path, caption = "", quoted = "", gif = false, options = {}) => {
      const buffer = Buffer.isBuffer(path)
        ...
        ? fs.readFileSync(path)
        : Buffer.alloc(0);
      return await sock.sendMessage(jid, { video: buffer, caption, gifPlayback: gif, ...options }, { quoted });
    };
